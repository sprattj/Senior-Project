...
export default class InventoryScreen extends React.Component {
    ...
    /**
     * Delete a container from the backend and remove it from the frontend display
     * @param {int} item_id - the id of an item with an item_type of container
     */
    deleteContainer(itemInfo, containerInfo)
    {
        this.containerDelete(itemInfo, containerInfo);
        // Call in above funct: this.handleItemDelete(itemInfo, ITEM_TYPES.CONTAINER);
    }
    ...
    containerDelete(itemInfo, containerInfo)
    {
        this.itemDelete(ITEM_TYPES.CONTAINER, itemInfo, containerInfo);
    }
    ...
    itemDelete(itemType, itemInfo, specificInfo)
    {
        var combinedItemInfo = this.getCombinedItem(itemType, itemInfo, specificInfo);
        this.handleItemDelete(combinedItemInfo, itemType);
    }
    ...
    */
    ...
    /** 
     * Make a delete request to remove an item. itemID passed determines which item to be deleted.
    * @param {int} item_id - an id for the item to be deleted
    * @param {string} itemType - the type of the item
    */
    handleItemDelete(itemInfo, itemType) 
    {
        var itemTypeAccessor = ITEM_TYPE_ACCESSORS[itemType];
        var endpoint = itemTypeAccessor + "/" + itemInfo.item_id;
        var successMsg = itemType + " deleted successfully.";
        var errorMsg = "Problem deleting " + itemType + ". With Item ID: " + itemInfo.item_id;

        var self = this;
        var variables = itemInfo;
        variables.item_id = itemInfo.item_id;
        
        var item_id = variables.item_id;

        // PIN ISN'T NEEDED TO CHANGE ITEM INFO
        // variables.pin = this.state.pin;
        ...
        var callback = function (responseData) 
        {
            ...
            // if success == true from responseData then continue to delete from MAPS

            // console.log("responseData: " + JSON.stringify(responseData));
            console.log("item_id: " + item_id);

            console.log("found element w/item_id? " + self.all.has(item_id));

            self.all.delete(item_id);
            self[itemTypeAccessor].delete(item_id);

            // FOR DEBUGGING:
            var size = self.all.size;
            var allIter = self.all.values();
            var allIterKeys = self.all.keys();
            if (size > 0)
            {
                for (var i = 0; i < size; i++)
                {
                    console.log("\n all types, " + i + " element: key: " + JSON.stringify(allIterKeys.next().value) + " \n")
                    console.log("\n all types, "  + i + " element: value: " + JSON.stringify(allIter.next().value) + " \n");
    ...
    */
    containerSelected(row, itemInfo) {
        ...
        return <InventoryDisplayContainer
            ...
            deleteContainerRow={this.deleteContainer}
        />;
    }
